class RurumMessenger {
    ...
    handleBroadcastMessage(data) {
        ...
        switch (data.type) {
            ...
            case 'friend_response':
                ...
                break;
            case 'sync_request':
                // Запрос на синхронизацию - отправляем все заявки
    ...
    // НАЧАЛО: СИСТЕМА ЗАЯВОК В ДРУЗЬЯ
    ...
    sendFriendRequest() {
        ...
        console.log('1. На другом устройстве откройте это приложение');
        console.log(`2. Установите имя пользователя: "${friendName}"`);
        console.log('3. Нажмите кнопку "Проверить заявки" или перезагрузите страницу');
    }
    ...
    updateFriendRequestsIndex(requestId, toUserName) {
        try {
            ...
            // Ограничиваем количество заявок на пользователя (10 максимум)
    ...
    saveToLocalStorageImmediate(type, data) {
        try {
            ...
            // Имитируем событие storage для других вкладок
            setTimeout(() => {
    ...
    checkStoredFriendResponses() {
        try {
            ...
            console.log('Found response keys:', responseKeys);
...
window.clearData = () => {
    if (confirm('Очистить все данные? Это перезагрузит страницу.')) {
        // Удаляем все ключи, начинающиеся с rurum
        const allKeys = Object.keys(localStorage);
        allKeys.forEach(key => {
            if (key.startsWith('rurum')) {
                localStorage.removeItem(key);
...
// Функция для принудительной проверки заявок
window.checkFriendRequestsNow = () => {
    console.log('🔍 Manually checking friend requests...');
...
window.createTestFriendRequest = () => {
    ...
    if (fromName && toName) {
        ...
        console.log('✅ Test friend request created:', testRequest);
    }
};

// Функция для очистки только заявок
window.clearFriendRequests = () => {
    if (confirm('Очистить все заявки?')) {
        // Удаляем все ключи, связанные с заявками
        const allKeys = Object.keys(localStorage);
        allKeys.forEach(key => {
...
window.addEventListener('beforeunload', () => {
    if (messenger) {
        messenger.stopRequestsCheckInterval();
    }
});