class RurumMessenger {
    ...
    init() {
        ...
        // Проверяем демо-заявки при загрузке
        setTimeout(() => this.checkDemoRequests(), 1000);
        setTimeout(() => this.checkDemoResponses(), 1500);
    ...
    setupChatChannels() {
        // Подписываемся на все активные чаты
    ...
    sendFriendRequest() {
        ...
        this.sentRequests.push(sentRequest);
        this.saveToStorage();
        this.hideAddFriendModal();
        
        // Сохраняем заявку в общее хранилище для демо
        this.saveFriendRequestToSharedStorage(sentRequest);
    ...
    saveFriendRequestToSharedStorage(request) {
        try {
            // Используем localStorage для симуляции работы между устройствами
            // В реальном приложении это было бы на сервере
            const allRequests = JSON.parse(localStorage.getItem('rurum_all_requests') || '[]');
            ...
            localStorage.setItem('rurum_all_requests', JSON.stringify(allRequests));
            console.log('💾 Friend request saved to shared storage');
            
        } catch (error) {
            console.error('❌ Error saving friend request to shared storage:', error);
        }
    }
    ...
    checkDemoRequests() {
        try {
            ...
            console.log('🔍 Checking for friend requests addressed to:', myName);
            console.log('All requests in storage:', allRequests);
            
            // Ищем заявки, адресованные текущему пользователю
            const requestsForMe = allRequests.filter(req => 
                req.type === 'friend_request' && 
                req.toUserName === myName && 
                !this.friendRequests.find(r => r.id === req.id) &&
                req.fromUserId !== this.currentUser.id // Не наши собственные заявки
            );
            
            console.log('Found requests for me:', requestsForMe);
            
            if (requestsForMe.length > 0) {
                // Добавляем найденные заявки
                requestsForMe.forEach(request => {
                    const newRequest = {
                        id: request.id,
                    ...
                    if (!this.friendRequests.find(req => req.id === newRequest.id)) {
                        this.friendRequests.push(newRequest);
                        console.log('📨 Added new friend request:', newRequest);
                ...
                // Удаляем обработанные заявки из общего хранилища
                const remainingRequests = allRequests.filter(req => 
                    !requestsForMe.find(r => r.id === req.id)
                );
                localStorage.setItem('rurum_all_requests', JSON.stringify(remainingRequests));
                
                // Обновляем список заявок, если модальное окно открыто
                if (document.getElementById('friendRequestsModal').style.display === 'flex') {
    ...
    acceptFriendRequest(requestId) {
        ...
        // Сохраняем ответ в общее хранилище
        this.saveFriendResponseToSharedStorage(requestId, true, request.fromUserId, request.fromUserName);
    ...
    rejectFriendRequest(requestId) {
        ...
        // Сохраняем ответ в общее хранилище
        this.saveFriendResponseToSharedStorage(requestId, false, request.fromUserId, request.fromUserName);
        
        this.showMessage(`Вы отклонили заявку от ${request.fromUserName}`, 'info');
    }

    saveFriendResponseToSharedStorage(requestId, accepted, toUserId, toUserName) {
        try {
            const allRequests = JSON.parse(localStorage.getItem('rurum_all_requests') || '[]');
            
            // Добавляем ответ на заявку
            allRequests.push({
                id: requestId,
                accepted: accepted,
                fromUserId: this.currentUser.id,
            ...
            localStorage.setItem('rurum_all_requests', JSON.stringify(allRequests));
            console.log('💾 Friend response saved to shared storage');
            
        } catch (error) {
            console.error('❌ Error saving friend response to shared storage:', error);
        }
    }
    ...
    checkDemoResponses() {
        try {
            ...
            console.log('🔍 Checking for friend responses for user:', myId);
            
            // Ищем ответы на наши заявки
            const responsesForMe = allRequests.filter(req => 
                req.type === 'friend_response' && 
                req.toUserId === myId
            );
            
            console.log('Found responses for me:', responsesForMe);
            
            if (responsesForMe.length > 0) {
                // Обрабатываем ответы
                responsesForMe.forEach(response => {
                ...
                // Удаляем обработанные ответы из общего хранилища
                const remainingRequests = allRequests.filter(req => 
                    !responsesForMe.find(r => r.id === req.id)
                );
                localStorage.setItem('rurum_all_requests', JSON.stringify(remainingRequests));
            }
            
        } catch (error) {
            console.error('❌ Error checking demo responses:', error);
        }
    }

    // ОСНОВНЫЕ МЕТОДЫ МЕССЕНДЖЕРА
    bindEvents() {
...
// Функция для симуляции тестовой заявки
window.simulateFriendRequest = () => {
    ...
    if (fromName && toName) {
        const requestId = 'simulated_' + Date.now();
        const simulatedRequest = {
            id: requestId,
            type: 'friend_request',
            fromUserId: 'simulated_' + fromName,
        ...
        try {
            const allRequests = JSON.parse(localStorage.getItem('rurum_all_requests') || '[]');
            allRequests.push(simulatedRequest);
            localStorage.setItem('rurum_all_requests', JSON.stringify(allRequests));
            
            alert(`Тестовая заявка от "${fromName}" к "${toName}" создана.\n\nНа устройстве пользователя "${toName}" перезагрузите страницу чтобы увидеть заявку.`);
        } catch (error) {
...
// Функция для проверки общего хранилища
window.checkSharedStorage = () => {
    const allRequests = JSON.parse(localStorage.getItem('rurum_all_requests') || '[]');
    console.log('📋 Shared storage contents:', allRequests);
    alert(`В общем хранилище ${allRequests.length} записей:\n\n${JSON.stringify(allRequests, null, 2)}`);
};
